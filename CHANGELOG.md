...
- Adds a first-run desktop shortcut prompt that creates the shortcut from inside the app.
- Opens first-run setup in a clickable normal window, then restores the user's previous widget settings.
- Redesigns the About window with a cleaner release/status layout.
- Keeps visualizer-only widgets visible with subtle idle motion when audio is silent.
